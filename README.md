...
### Тесты контента и поиска

- **Секция Topics** (`test_topics.py`) — на **github.com/topics**: собираем
  slug'и всех карточек-топиков и проверяем, что нужные есть среди них
  (**subset**-проверка `set.issubset` — устойчиво к смене подборки).
- **Параметризованный поиск** (`test_search.py`) — на **duckduckgo.com**
  (plan B: стабильнее google, без reCAPTCHA). Один тест на запросах
  `qa / aqa / python`, приёмка — результатов больше 5 (`nth(5)` видим).